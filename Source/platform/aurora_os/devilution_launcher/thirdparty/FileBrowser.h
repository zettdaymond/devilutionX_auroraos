...
inline void ImGui::FileBrowser::Display()
{
    ...
    {
        if(!BeginPopup(openLabel_.c_str(), windowFlags))
        {
            return;
        }
    }
    else if(!BeginPopupModal(openLabel_.c_str(), nullptr,
                             flags_ & ImGuiFileBrowserFlags_NoTitleBar ? (ImGuiWindowFlags_NoTitleBar | windowFlags) : windowFlags))
    {
        return;
    }
...
{
    const std::string folderStr = ICON_FA_FOLDER; //m_font_awesome ? ICON_FA_FOLDER : "[D]";
    const std::string fileStr = ICON_FA_FILE; //m_font_awesome ? ICON_FA_FILE : "[F]";

    fileRecords_ = { FileRecord{ true, "..", folderStr, " ..", "" } };
    ...
    for(auto &p : std::filesystem::directory_iterator(currentDirectory_))
    {
        ...
        try
        {
            ...
            rcd.icon = rcd.isDir ? folderStr : fileStr;
