...
private:
	...
	/// ApplyTileSize. Вызывается после успешного линка обложки. Перед
	/// запуском проверяет кэш ответа — на знакомом устройстве зонд
	/// не нужен вовсе.
	void StartCoverProbe();

	/// Кэш ответа зонда в [Cover] launcher.ini: размер плитки — константа
	/// Silica для связки «устройство + ОС». Контекст валидности — версия
	/// ОС (/etc/os-release) и логический экран: расхождение снова зовёт
	/// зонд. true — кэш валиден и применён.
	[[nodiscard]] bool TryApplyCoverCache();

	/// Записать кэш ответа зонда (главный поток, по событию ответа).
	void WriteCoverCache(int tileWidth, int tileHeight);

	/// Каталог конфигов лаунчера (launcher.ini); заполняет только
	/// конструктор с настоящими сервисами.
	...
	/// Событие «Qt-зонд ответил размером плитки» (фоновый поток пушит
	/// его после coverprobe; data1/data2 — ширина/высота).
