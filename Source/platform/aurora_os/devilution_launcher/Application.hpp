...
private:
	...
	/// coverstatus не адресован окну: жест считаем своим, только если
	/// его начало (значение 2) пришло, пока мы передний план (жест бывает
	/// только на переднем плане); чужие жесты в фоне метку обнуляют.
	/// Возврат (3/0) выводит из обложки мгновенно, только если жест был наш.
	bool m_coverGestureOurs = false;
