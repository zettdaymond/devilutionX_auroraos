...
private:
	...
	/// Запечатать кадр обложки для фазы движка (GameCover): на Авроре
	/// после «Играть» окно лаунчера умирает, а движок в плитке показывает
	/// фирменную обложку — пиксели снимаются с нашего рендерера.
	void BakeGameCover();
