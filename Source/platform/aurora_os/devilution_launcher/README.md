...
Внешний вид карточки — три состояния на тёплом угле, без фото-арта
(шапку с иконкой и именем рисует композитор): простой лаунчера — лицо
режима (череп Diablo / монах Hellfire); загрузка — эмбер-кольцо с
процентом (H1-акцент канона Авроры), именем файла и скоростью; пауза
(игра запущена) — призрачное лицо и дышащий уголёк за ним (синус ~3.5 с,
без слова и символа паузы). Превью на десктопе: `--cover-preview`,
`--cover-pause` (синтетическое состояние паузы), `--cover-download`;
дизайн и фолбэки для стора — в `aurora_project/docs/CoverResearch.md`.
