...
#ifdef AURORA_OS
	// Финальный живой кадр: плитка на фазе движка показывает статичную
	// карточку состояния лаунчера на момент запуска игры (живая
	// анимация — после GPU-рендера обложки, WIP 4b).
	UpdateNativeCover();
	m_stateWatch.reset();
#endif
