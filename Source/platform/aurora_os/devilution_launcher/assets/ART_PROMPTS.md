...
**Компрессия светов (bg.jpg, 2026-09-01):** жёлтые огни деревни слепили на
фоне текста (max 237, доля >140 — 0.29%). Лечится в HSV мягким плечом
`tanh`: `V' = Vc·tanh(V/Vc)` — монотонная кривая, тени не трогает (при
малых V ≈ тождественна), верх сатурируется. Финал — **Vc=0.30** по
градации 0.55/0.40/0.35/0.30/0.25: max 73, p99.9 66, mean 11.5 — огни
сравнялись с пеленой тумана, текст читается идеально, сцена не отвлекает
(философия: фон — дымка, внимание забирают hero-арты). **Важно:** маски
вида `V·(1−k·smoothstep(t, V))` НЕмонотонны — ядро огня прижимается
сильнее кромки свечения, свечение «выворачивается» и вокруг окон
расцветают жёлтые кольца (проверено на Vc=0.55-подходе). **Оригинал без
компрессии хранится рядом: `bg_alt_village_d68.jpg`** (для пересборки
другой градации).
