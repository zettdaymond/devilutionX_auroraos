...
`tanh`: `V' = Vc·tanh(V/Vc)`, Vc=0.55 — монотонная кривая, тени не трогает
(при малых V ≈ тождественна), верх сатурируется: max 125, доля >140 — 0%,
mean 13.6 → 12.1. **Важно:** маски вида `V·(1−k·smoothstep(t, V))`
НЕмонотонны — ядро огня прижимается сильнее кромки свечения, свечение
«выворачивается» и вокруг окон расцветают жёлтые кольца (проверено).
**Оригинал без компрессии хранится рядом: `bg_alt_village_d68.jpg`.**
