...
Сгенерированы там же (сида 4711–4713, 1024×1024, те же параметры, но CFG 5 /
12 шагов и жёсткий промпт «logo emblem, solid black flat silhouette, stencil»).
Постобработка (порог + флуд-филл от краёв) превращает драфт в плоское
золото #E8C77E с альфой и заполняет внутренние детали; скважина Demo
пробита в маске кодом. Файлы: `icon_diablo.png` (рогатый череп),
`icon_hellfire.png` (монах в капюшоне), `icon_demo.png` (арка со скважиной),
256×256 RGBA. Фолбэк при отсутствии — FontAwesome-глифы (Fire/Gamepad/Download).
DreamShaper Lightning при CFG 2 игнорирует «flat silhouette» — нужен CFG ~5.
