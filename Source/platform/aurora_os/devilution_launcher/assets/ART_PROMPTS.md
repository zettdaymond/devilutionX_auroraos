...
фоне текста (max 237, доля >140 — 0.29%). Лечится в HSV без трогания теней:
V умножается на `1 − 0.55·smoothstep(0.42, 0.62, V)`, насыщенность ярких
зон на `1 − 0.30·smoothstep(0.42, 0.65, V)` (жёлтый → амбер). Итог:
max 109, доля >140 — 0%, mean 13.6 → 13.2. **Оригинал без компрессии
хранится рядом: `bg_alt_village_d68.jpg`.**
