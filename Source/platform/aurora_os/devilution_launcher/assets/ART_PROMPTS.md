...
Кодирование (2026-09-01): JPEG на устройстве недоступен — статическая
libSDL2_image.a сборки Aurora не содержит JPEG-символов (проверено nm).
Вместо этого арты хранятся палитровыми PNG (256 цветов, дизеринг
Floyd–Steinberg): bg 2048×1200 (1,5→1,1 МБ), heroes 960×657 (по ~1,3→0,4 МБ);
 heroes больше 960px на экране не отображаются, а декодированная текстура
ужалась на ~37%. Непрозрачные арты конвертируются в RGB24 перед созданием
текстуры. Пайплайн пересжатия: PIL quantize(MEDIANCUT) + resize LANCZOS.
