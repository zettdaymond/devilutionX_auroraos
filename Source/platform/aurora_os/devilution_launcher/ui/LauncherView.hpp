...
public:
	...
	/// Кадр «обложки» свёрнутого приложения (плитка Авроры): фирменный
	/// фон, логотип и прогресс активной загрузки. Без окон и ввода —
	/// композитор показывает буфер в плитке, тап по которой лишь
	/// открывает приложение. `coverAlpha` < 1 — кадр кросс-фейда:
	/// обложка полупрозрачна поверх только что отрисованного интерфейса.
	void RenderCover(const LauncherState &state, float coverAlpha = 1.0F);

	/// Окно снова видимо после плитки. Кадры обложки не содержат
	/// ImGui-окон, и открытый модальный диалог за них умирает —
	/// вызов заставляет следующий Render переоткрыть активный диалог.
	void NotifyShown();
