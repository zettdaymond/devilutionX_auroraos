...
public:
	...
	/// Спячка свёрнутого состояния для честных пауз движка: ждёт любое
	/// событие; тики цикла дыхания обложки ПОГЛОЩАЕТ (возврат их в
	/// очередь оживляет WaitEvent — цикл сайта крутится вхолостую на
	/// полное ядро), прочие события возвращает в очередь, как раньше.
	static void SleepWhileHidden();
};
