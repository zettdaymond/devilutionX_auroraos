...
public:
	...
	/// Применить размер плитки, добытый Qt-зондом (Silica Theme):
	/// приоритет ниже configure (серверная правда), выше фолбэка.
	/// Пересоздаёт SHM-пул и перезаливает последний кадр.
	static void ApplyTileSize(int width, int height);
