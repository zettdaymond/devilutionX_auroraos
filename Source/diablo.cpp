...
#ifdef AURORA_OS
		// Плитка/гашение: игра честно спит — логика и рендер не гоняются
		// (как у меню и видео). RenderPresent кормит машину обложки и
		// держит карточку; спим до события, проснутое возвращаем в
		// очередь — его обработает насос наверху цикла. Без этого цикл
		// крутился вхолостую на ~90% CPU: vsync не душит (кадр не
		// презентится), логика стоит (PauseMode), а цикл — нет.
