# -*- coding: utf-8 -*-
"""Light-постобработка img2img вариантов: они уже тёмные и в палитре."""
import colorsys
import io
import os
...
def process(path, tag):
    ...
    img.save(os.path.join(OUT, "proc3_{}.jpg".format(tag)), "JPEG", quality=90)
    palette_stats(img, "proc3_" + tag)
    ...
    for orient, aspect in (("land", 16 / 9), ("port", 9 / 19.5)):
        ...
        prev.save(os.path.join(OUT, "prev3_{}_{}.jpg".format(tag, orient)), "JPEG", quality=88)


palette_stats(Image.open(os.path.join(ASSETS, "bg.jpg")), "bg.jpg  ")
for tag in ("i2i_bg062_8301", "i2i_bg062_8302", "i2i_bg075_8301", "i2i_wallpan045_8301"):
    process(os.path.join(SRC, tag + ".png"), tag)
print("done ->", OUT)
